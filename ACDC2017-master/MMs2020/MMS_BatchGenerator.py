...
class BatchGenerator_2D(DataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        types = np.random.choice(['ed', 'es'], self.BATCH_SIZE, True)  # randomly choose es or ed
        # print(self._data.keys())
        patients = np.random.choice(list(self._data.keys()), self.BATCH_SIZE, True)  # randomly choose 'batchsize' patients
        # pathologies = []
        for nb in range(self.BATCH_SIZE):
            """
            data[i]['ed_data'] = a[0, :]
            ...
            """
            shp = self._data[patients[nb]][types[nb]+'_data'].shape
            slice_id = np.random.choice(shp[0])  # randomly choose one slice from total slice, here is 10
            ...
            # pathologies.append(self._data[patients[nb]]['pathology'])
            # print(data)
        return {'data':data, 'seg':seg, 'types':types, 'patient_ids': patients}
